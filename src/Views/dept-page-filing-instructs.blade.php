...
<div class="slCard mT20">
    <h3 class="mT0">
        How to File Complaints against the {!! $d["deptRow"]->DeptName !!}
    </h3>
    @if (isset($d["iaRow"]->OverOfficialFormNotReq) 
        && intVal($d["iaRow"]->OverOfficialFormNotReq) == 1
        && isset($d["iaRow"]->OverWaySubEmail) 
        && intVal($d["iaRow"]->OverWaySubEmail) == 1
    ...
    @else
        @if (isset($d["iaRow"]->OverWaySubPaperInPerson) 
...
@if ($d["whichOver"] == 'civRow')
    ...
    against the {!! $d["deptRow"]->DeptName !!}.
...
@endif
    ...
    && trim($d[$d["whichOver"]]->OverPhoneWork) != '')
        <p>
            ...
            {{ $d[$d["whichOver"]]->OverPhoneWork }}
        </p>
    @endif
    
    @if (isset($d["iaRow"]->OverWaySubEmail) 
        && intVal($d["iaRow"]->OverWaySubEmail) == 1
        && isset($d[$d["whichOver"]]->OverEmail) 
        && trim($d[$d["whichOver"]]->OverEmail) != '')
        @if (!isset($d["iaRow"]->OverOfficialFormNotReq) 
            || intVal($d["iaRow"]->OverOfficialFormNotReq) == 0)
            <p>
        ...
        @endif
    @elseif (isset($d[$d["whichOver"]]->OverEmail) 
        && trim($d[$d["whichOver"]]->OverEmail) != ''
        && (!isset($d[$d['whichOver']]->OverKeepEmailPrivate) 
            || intVal($d[$d['whichOver']]->OverKeepEmailPrivate) == 0))
    ...
    @if (isset($d["iaRow"]->OverComplaintPDF) 
        && trim($d["iaRow"]->OverComplaintPDF) != '')
