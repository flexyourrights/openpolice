...
@elseif ($GLOBALS["SL"]->def->getVal('Complaint Status', $complaint->ComStatus) 
    ...
    @if ($complaint->ComAnyoneCharged == 'Y' 
        && in_array($complaint->ComAllChargesResolved, ['N', '?']))
...
@elseif ($GLOBALS["SL"]->def->getVal('Complaint Status', $complaint->ComStatus) 
    ...
    @if (isset($oversights) && sizeof($oversights) > 0 && isset($oversights[0]->OverEmail) 
        && trim($oversights[0]->OverEmail) != '' && isset($oversights[0]->OverWaySubEmail) 
        && intVal($oversights[0]->OverWaySubEmail) == 1)
    ...
    @else
        ...
        @if (isset($oversights) && sizeof($oversights) > 0 
            ...
            && intVal($oversights[0]->OverWaySubPaperInPerson) == 1)
            You
        @else
...
@if (!isset($complaint->ComStatus) || intVal($complaint->ComStatus) <= 0 
    ...
    <a href="/switch/1/{{ $complaint->ComID }}" id="ownBtnCont" 
    ...
    <a href="javascript:;" class="btn btn-lg btn-danger w100 taL" id="ownBtnDel" 
        ...
        onClick="if (confirm('{!! $warning !!}')) { window.location='/delSess/1/{{ $complaint->ComID }}'; }" style="color: #FFF;"
