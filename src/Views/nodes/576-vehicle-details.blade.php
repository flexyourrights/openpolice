...
<div id="node{{ $nodes[0] }}" class="nodeWrap">
	...
	<div class="nFld @if (sizeof($vehicles) == 0) disNon @endif ">
		<div class=" disIn mR20">
			<label for="n{{ $nodes[0] }}fld0" class="mR10">
				<div class="disIn mR5"><input id="n{{ $nodes[0] }}fld0" value="Y" type="radio" name="n{{ $nodes[0] }}fld"  class="n{{ $nodes[0] }}fldCls"  autocomplete="off" 
					onClick="checkNodeUp();" @if ($alreadyDescribed == 'Y') CHECKED @endif ></div> Yes
			</label>
		</div>
		<div class=" disIn mR20">
			<label for="n{{ $nodes[0] }}fld1" class="mR10">
				<div class="disIn mR5"><input id="n{{ $nodes[0] }}fld1" value="N" type="radio" name="n{{ $nodes[0] }}fld"  class="n{{ $nodes[0] }}fldCls"  autocomplete="off" 
	...
	<div id="node{{ $nodes[0] }}kidsY" class="nKids @if ($alreadyDescribed == 'Y') disBlo @else disNon @endif ">
		<input type="hidden" name="n{{ $nodes[1] }}Visible" id="n{{ $nodes[1] }}VisibleID" @if ($alreadyDescribed == 'Y') value="1" @else value="0" @endif >
		<div class="fC"></div>
				...
				@forelse ($vehicles as $i => $v)
					<div class="">
						<nobr><label for="n{{ $nodes[1] }}fld{{ $i }}" class="mR10">
							<div class="disIn mR5"><input id="n{{ $nodes[1] }}fld{{ $i }}" value="{!! $v[0] !!}" type="radio" name="n{{ $nodes[1] }}fld" autocomplete="off" 
