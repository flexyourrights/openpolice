...
@if (isset($allUrls) && sizeof($allUrls) > 0 && sizeof($allUrls["vid"]) > 0)
    ...
    @foreach ($allUrls["vid"] as $i => $url)
        ...
        @endif
            <div class="w100 round10">
                <iframe frameborder="0" allowfullscreen style="width: 100%;
                    @if (!in_array(sizeof($allUrls['vid']), [1, 2, 4])) height: 200px; 
                    @else height: 315px; 
                    @endif " src="https://www.youtube.com/embed/{{ 
                        $GLOBALS['SL']->getYoutubeID($url[1]) 
                    }}?rel=0&color=white" 
                    ></iframe>
            </div>
        </div>
...
@else
    <style> #node1752 { display: none; } </style>
@endif
