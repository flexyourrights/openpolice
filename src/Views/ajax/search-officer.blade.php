...
@forelse($offs as $row)
    <a href="javascript:void(0)" onClick="offvLoad('{{ htmlspecialchars($row->OffVNameFirst) }}', '{{ htmlspecialchars($row->OffVNameMiddle) }}', '{{ htmlspecialchars($row->OffVNameLast) }}', '{{ htmlspecialchars($row->OffVBadgeNumber) }}', '{{ htmlspecialchars($row->OffVRank) }}');" 
