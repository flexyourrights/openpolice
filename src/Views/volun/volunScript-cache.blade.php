<div class="w100 ovrFlo pL5">
    <div class="card">
        <div class="card-header">
            Main Department Phone Script 
            <a class="float-right" href="/dashboard/volunteer/verify/checklist#script" 
                target="_blank"><i class="fa fa-external-link"></i></a>
        </div>
        <div class="card-body slBlueDark">
            {!! $script1 !!}
        </div>
    </div>
    <div class="card">
        <div class="card-header">Internal Affairs Phone Script</div>
        <div class="card-body slBlueDark">
            {!! $script2 !!}
        </div>
    </div>
</div>

<style>
#rightSide .ovrFlo { 
    height: 600px; 
    border-bottom: 1px #AAA dashed; 
}
#rightSide .ovrFlo .panel.panel-info .card-body { 
    font-size: 14px; 
}
#rightSide .ovrFlo .panel.panel-info .card-body ol,
#rightSide .ovrFlo .panel.panel-info .card-body ul { 
    margin: 0px; 
}
#rightSide .ovrFlo .panel.panel-info .card-body ol li, 
#rightSide .ovrFlo .panel.panel-info .card-body ul li { 
    margin: 0px; 
    margin-left: -30px; 
    padding-bottom: 20px; 
}
</style>