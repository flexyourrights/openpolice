...
class OpenPoliceAdmin extends AdminSubsController
{
	...
	public function volunDepts(Request $request)
	{
		...
		$statRanges[] = array('Last 24 Hours', " WHERE `EditDeptVerified` > '" . date("Y-m-d H:i:s", mktime(date("H")-24, date("i"), date("s"), date("n"), date("j"), date("Y"))) . "'");
		$statRanges[] = array('This Week', " WHERE `EditDeptVerified` > '" . date("Y-m-d H:i:s", mktime(date("H"), 0, 0, date("n"), date("j")-7, date("Y"))) . "'");
	...
	public function recalcVolunStats()
	{
        ...
        if ($edits && sizeof($edits) > 0)
        {
        	foreach ($edits as $i => $e)
        	{
        		...
        		if (!in_array($e->EditOverUser, $days[$day]["users"])) 		$days[$day]["users"][] = $e->EditOverUser;
        		if (!in_array($e->EditOverDeptID, $days[$day]["depts"])) 	$days[$day]["depts"][] = $e->EditOverDeptID;
        	}
        }
	...
	protected function loadPrintVoluns()
	{
		...
		$volunteers = User::where('name', 'NOT LIKE', 'Session#%')->orderBy('name', 'asc')->get();
		foreach ($volunteers as $i => $volun) {
			$list = 3;
